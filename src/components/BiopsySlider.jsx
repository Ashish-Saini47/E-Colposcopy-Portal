...
class BiopsySlider extends React.Component {
  constructor(){
    super();
    this.state={
      images:[],
  ...
  componentDidMount(){
    const imageRef = ref(db, "DoctorsList/"+localStorage.getItem("currentUserId")+"/PatientList/" + localStorage.getItem("currentPatientId") +"/Biopsy")
    let date = []
    let imageUrl = []
    onValue(imageRef, (dateSnap)=>{
      dateSnap.forEach((dateChild)=>{
        ...
        const dataRef = ref(db, "DoctorsList/"+localStorage.getItem("currentUserId")+"/PatientList/" + localStorage.getItem("currentPatientId") +"/Biopsy/" + dateChild.key)
  ...
  render(){
    return (
      <div className='bg-white w-full p-10'>
        <h1 className="text-3xl font-medium text-gray-500"><span className="text-emerald-500">L</span>ast <span className="text-emerald-500">B</span>iopsy  <span className="text-emerald-500">I</span>mage</h1>
          {
            this.state.images.length ==0 ? <div className='flex justify-center text-2xl text-gray-600 pb-10 mt-5 lg:mt-0'> No Biopsy Image Found For This Patient</div> :
            <div className='w-full flex gap-3 justify-center items-center lg:px-14  pt-10'>
